...
void TravelNodeMap::saveNodeStore()
{
    ...
    PlayerbotDatabase.PExecute("DELETE FROM playerbot_travelnode");
    PlayerbotDatabase.PExecute("DELETE FROM playerbot_travelnode_link");
    PlayerbotDatabase.PExecute("DELETE FROM playerbot_travelnode_path");
    ...
    for (uint32 i = 0; i < anodes.size(); i++)
    {
        ...
        PlayerbotDatabase.PExecute("INSERT INTO `playerbot_travelnode` (`id`, `name`, `map_id`, `x`, `y`, `z`, `linked`) VALUES ('%lu', '%s', '%d', '%f', '%f', '%f', '%d%')"
...

    {
        ...
        for (uint32 i = 0; i < anodes.size(); i++)
        {
            ...
            for (auto& link : *node->getLinks())
            {
                ...
                PlayerbotDatabase.PExecute("INSERT INTO `playerbot_travelnode_link` (`node_id`, `to_node_id`,`type`,`object`,`distance`,`swim_distance`, `extra_cost`,`calculated`, `max_creature_0`,`max_creature_1`,`max_creature_2`) VALUES ('%lu','%lu', '%d', '%lu', '%f', '%f', '%f', '%d', '%d', '%d', '%d')"
                    , i
                ...
                for (uint32 j = 0; j < ppath.size(); j++)
                {
                    ...
                    PlayerbotDatabase.PExecute("INSERT INTO `playerbot_travelnode_path` (`node_id`, `to_node_id`, `nr`, `map_id`, `x`, `y`, `z`) VALUES ('%lu', '%lu', '%d','%d', '%f', '%f', '%f')"
                        , i
                        , saveNodes.find(link.first)->second
                        , j
                        , point.getMapId()
...
{
    string query = "SELECT id, name, map_id, x, y, z, linked FROM playerbot_travelnode";
...

    {
        ...
        string query = "SELECT node_id, to_node_id,type,object,distance,swim_distance, extra_cost,calculated, max_creature_0,max_creature_1,max_creature_2 FROM playerbot_travelnode_link";
    ...
    {
        //                     0        1           2   3      4   5  6
        string query = "SELECT node_id, to_node_id, nr, map_id, x, y, z FROM playerbot_travelnode_path order by node_id, to_node_id, nr";
